<script>
  ...
  import { app, toast, METRICS, SUMMARY_METRICS } from '../lib/store.svelte.js';
  ...
  const isOver = (key) => ['high', 'hazard'].includes(metricOf(key)?.level);
...
{:else}
  ...
  <div class="grid readings">
    {#each SUMMARY_METRICS as key (key)}
      {@const m = METRICS[key]}
      {@const over = isOver(key)}
      {@const value = latest?.[key]}
      <div class="plate reading">
        ...
        <span class="reading__hint" style:color={over ? 'var(--warn)' : 'var(--faint)'}>
          {key === 'tds' ? 'ПДК 1000' : `норма ${m.norm[0]}–${m.norm[1]}`}
        </span>
      </div>
    {/each}
  </div>
...
<style>
  ...
  .readings { grid-template-columns: repeat(2, minmax(0, 1fr)); margin-bottom: var(--gap); }
  ...
  @media (min-width: 900px) {
    ...
    .charts { grid-template-columns: minmax(0, 1.55fr) minmax(0, 1fr); }
