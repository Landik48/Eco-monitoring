<script>
  ...
  const GROUPS = [
    ...
    { title: 'Контроль достоверности',
      hint: 'Пороги, по которым станция считает показания недостоверными',
      keys: ['vbat_min', 'vbat_sag_delta', 'temp_rate_per_min', 'pres_rate_per_min', 'tds_rate_per_min'] }
  ];

  const GROUPED = new Set(GROUPS.flatMap((g) => g.keys));

  const extraKeys = $derived(Object.keys(schema).filter((k) => !GROUPED.has(k)));

  const allGroups = $derived(
    extraKeys.length
      ? [...GROUPS, { title: 'Прочее', hint: 'Параметры станции без отдельного раздела', keys: extraKeys }]
      : GROUPS
  );
